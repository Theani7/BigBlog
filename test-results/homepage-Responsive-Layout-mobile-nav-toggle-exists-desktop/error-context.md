...
- Name: homepage.spec.ts >> Responsive Layout >> mobile nav toggle exists
- Location: tests/e2e/homepage.spec.ts:95:3
...
Locator: locator('button[aria-label*="menu" i], button[aria-label*="nav" i], [class*="mobile"]').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button[aria-label*="menu" i], button[aria-label*="nav" i], [class*="mobile"]').first()
